...
import { Dog, Syringe, AlertTriangle, CheckCircle, Clock, Plus, ArrowRight, PawPrint } from 'lucide-react';
...
function StatusBadge({ status, label }: { status: string; label: string }) {
  return (
    <span className={`status-${status} inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium`}>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div><div className="skeleton skeleton-text w-40" /><div className="skeleton skeleton-text-sm w-64 mt-2" /></div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
            Overview of your pets&apos; vaccination status
          </p>
        </div>
        <Link href="/dogs/new" className="btn-primary flex items-center gap-2 w-fit">
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl animate-slide-down"
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: '#f87171' }}>
              {stats.overdue} vaccination{stats.overdue !== 1 ? 's' : ''} overdue!
            </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 stagger-children">
        {[
          { label: 'Total Dogs', value: dogs.length, icon: Dog, color: 'var(--color-primary)', bg: 'rgba(245,158,11,0.08)' },
          { label: 'Up to Date', value: stats.upToDate, icon: CheckCircle, color: 'var(--color-success)', bg: 'rgba(16,185,129,0.08)' },
          { label: 'Due Soon', value: stats.dueSoon, icon: Clock, color: 'var(--color-warning)', bg: 'rgba(245,158,11,0.08)' },
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>My Dogs</h2>
          {dogs.length > 0 && (
            <Link href="/dogs" className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: 'var(--color-primary)' }}>
        ...
        {dogs.length === 0 ? (
          <div className="glass-card p-10 text-center" style={{ cursor: 'default' }}>
            ...
            <p className="text-sm font-semibold mb-1" style={{ color: 'var(--text-primary)' }}>No dogs added yet</p>
            <p className="text-xs mb-5" style={{ color: 'var(--text-muted)' }}>
              Add your first dog to start tracking vaccinations and health records.
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 stagger-children">
            {dogs.map((dog) => {
              const dogRecords = records.filter((r) => r.dogId === dog.id && r.status === 'completed');
              ...
              const protectionScore = totalRecords === 0 ? 0 : Math.round(((totalRecords - overdueCount - dueSoonCount) / totalRecords) * 100);
              
              let scoreColor = '#10b981'; // Green
              if (protectionScore < 100 && protectionScore >= 50) scoreColor = '#f59e0b'; // Yellow
              if (protectionScore < 50) scoreColor = '#ef4444'; // Red

              return (
                <Link key={dog.id} href={`/dogs/${dog.id}`} className="glass-card overflow-hidden group hover:scale-[1.02] transition-transform duration-300 flex flex-col">
                  {/* Banner/Image Area */}
                  <div className="h-32 w-full relative bg-slate-800/50 flex items-center justify-center overflow-hidden">
                    {dog.photoUrl ? (
                      <Image src={dog.photoUrl} alt={dog.name} fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
                    ) : (
                      <PawPrint size={40} className="text-slate-600 opacity-50" />
                    )}
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <p className="text-xs text-slate-400 mb-4 font-medium flex items-center gap-2">
                      <span className="truncate max-w-[120px]">{dog.breed}</span> • {getDogAge(dog.dateOfBirth)}
                    </p>
                    ...
                    <div className="mb-4 mt-auto">
                      <div className="flex justify-between text-[10px] uppercase tracking-wider font-bold mb-1.5">
                        ...
                        <span style={{ color: scoreColor }}>{protectionScore}%</span>
                      </div>
                      <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                        <div 
                          className="h-full transition-all duration-1000 ease-out rounded-full" 
                          style={{ width: `${protectionScore}%`, backgroundColor: scoreColor }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-auto">
                      {overdueCount > 0 && <StatusBadge status="red" label={`${overdueCount} overdue`} />}
                      {dueSoonCount > 0 && <StatusBadge status="yellow" label={`${dueSoonCount} due soon`} />}
                      {overdueCount === 0 && dueSoonCount === 0 && totalRecords > 0 && <StatusBadge status="green" label="All clear" />}
                      {totalRecords === 0 && <StatusBadge status="gray" label="Needs setup" />}
                    </div>
                  </div>
                </Link>
              );
        ...
        <div>
          ...
          <div className="space-y-2 stagger-children">
            {stats.upcoming.slice(0, 10).map((r) => {
              const info = getVaccinationStatus(r.nextDueDate);
              return (
                <Link key={r.id} href={`/dogs/${r.dogId}`} className="glass-card p-4 flex items-center justify-between">
